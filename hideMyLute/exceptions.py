...
class OperationCancelled(HideMyLuteError):
    """Операция отменена (пользователем или при завершении приложения).

    Возникает при проверке флага отмены внутри длительных файловых
    операций. Частично записанные файлы при этом удаляются.
    """

    def __init__(self) -> None:
        super().__init__(
            "Операция отменена",
            msg_key="error_operation_cancelled",
        )